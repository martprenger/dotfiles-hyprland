...
workspaces() {
  ...
  wsps="$(hyprctl workspaces -j | jq '.[] | del(select(.id == -99)) | .id' | jq -s 'sort' | jq '.[]')"
  awsps="$(hyprctl monitors -j | jq '.[] | select(.id).activeWorkspace.id')"
  test="(box	:orientation \"h\" :space-evenly \"false\""

  for i in ${wsps[@]}; do
    if [ $i == $awsps ]; then
      test+="(button :onclick \"scripts/dispatch.sh $i\" :class \"w0$i$awsps\" \"$i\")"
    else
      test+="(button :onclick \"scripts/dispatch.sh $i\" :class \"w0$i\" \"$i\")"
    fi
  done
  test+=")"
  echo $test
...
#echo 	"(eventbox :onscroll \"echo {} | sed -e 's/up/-1/g' -e 's/down/+1/g' | xargs hyprctl disp
# for num in $ows; do 
#	export o"$num"="$num"
#done
